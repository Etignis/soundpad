...
Vue.component('titem', {
	...
	computed: {
		style_class: function(){
			...
			// if(this.type='switch' && this.tumblr) {
				// aClass.push('active');
			// }
			return aClass.join(" ");
		}
  ...
  var app = new Vue({
    ...
    data: {
		...
		mounted: function() {
			...
			w.setSize(this.aWinSizes[this.sAppView].w,this.aWinSizes[this.sAppView].h);

			w.on('resize', function () {
				let size   = w.getSize();
				let width  = size[0];
				let height = size[1];
				this.aWinSizes[this.sAppView].w= width;
				this.aWinSizes[this.sAppView].h= height;
				
				this._saveData("aWinSizes");	
			}.bind(this));			
		},
		methods: {
			...
			toDefaultView: function(){
				...
				w.setSize(this.aWinSizes["default"].w,this.aWinSizes["default"].h);
			},
			toSquareView: function(){
				this.sAppView = "square"; 
				this._saveData("sAppView");	
				w.setSize(this.aWinSizes["square"].w,this.aWinSizes["square"].h);
			},
			toPanelView: function(){
				this.sAppView = "panel"; 
				this._saveData("sAppView");	
				w.setSize(this.aWinSizes["panel"].w,this.aWinSizes["panel"].h);
			},
